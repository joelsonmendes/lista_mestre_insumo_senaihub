...
function calculatePlan(input){
  ...
  for(const practice of coursePractices){
    for(const rel of state.practiceMaterials.filter(r=>r.practiceId===practice.id)){
      ...
      rows.push({practiceId:practice.id,practice:practice.name,materialId:m.id,code:m.code,protheusCode:m.protheusCode||'',name:m.name,category:m.category,unit:m.unit,raw:round(raw,3),needed:round(operationalQty,3),mobilize:round(mobilizeQty,3),stock:num(m.stock),buy:round(buyQty,3),wearQty:round(wearQty,3),unitPrice,quoteStatus:quoteStatus(p),immediateCost,wearCost,notes:rel.notes||''});
    }
  }
  const merged={};
...
function renderMaterials(){const q=$('#materialSearch').value.toLowerCase(),cat=$('#materialCategory').value;$('#materialsBody').innerHTML=state.materials.filter(m=>(!cat||m.category===cat)&&(`${m.code} ${m.name} ${m.spec}`).toLowerCase().includes(q)).map(m=>{const p=latestPrice(m.id),st=num(m.stock)<num(m.minStock)?'bad':num(m.stock)===num(m.minStock)?'warn':'ok';return`<tr><td>${esc(m.code)}</td><td>${esc(m.protheusCode||'—')}</td><td><b>${esc(m.name)}</b><br><small>${esc(m.spec||'')}</small></td><td>${esc(m.category)}</td><td>${esc(m.unit)}</td><td>${num(m.stock)} / mín. ${num(m.minStock)}</td><td>${num(m.lifeClasses)?`${m.lifeClasses} turmas`:'Consumo direto'}</td><td>${p?brl(p.unitPrice):'Pendente'}</td><td><span class="status ${st}">${st==='bad'?'Repor':st==='warn'?'No mínimo':'Adequado'}</span></td><td><button class="action-btn" data-edit-material="${m.id}">Editar</button></td></tr>`}).join('')}
...
function filteredReportItems(p,type){
  let items=[...(p.items||[])];
  if(type==='consumables')items=items.filter(r=>r.category==='Insumo consumível');
  if(type==='consumables_ppe')items=items.filter(r=>['Insumo consumível','EPI'].includes(r.category));
  if(type==='purchase')items=items.filter(r=>r.buy>0);
  if(type==='mobilization')items=items.filter(r=>r.mobilize>0);
  return items;
}
function reportTitle(type){return({complete:'Orçamento completo da turma',consumables:'Lista de insumos do curso',consumables_ppe:'Lista de insumos e EPIs',purchase:'Lista de materiais para compra',mobilization:'Lista de mobilização',return:'Checklist de retorno'})[type]||'Lista Mestre'}
function generateReport(){const p=state.plans.find(x=>x.id===$('#reportPlan').value);if(!p)return toast('Salve um planejamento primeiro.');const type=$('#reportType').value,items=filteredReportItems(p,type),title=reportTitle(type);$('#reportOutput').innerHTML=`<div class="report-brand"><img src="assets/logo-senai-hub.webp" alt="SENAI Hub"></div><div class="report-title"><h2>${title}</h2><b>${esc(p.courseName)} — ${p.hours} h</b><p>${p.students} alunos • ${p.groups} grupos • ${p.location==='externo'?'Execução externa':'Execução interna'} • ${esc(p.city||'')}</p></div><div class="report-totals"><div class="report-total">Desembolso imediato<strong>${brl(p.immediate)}</strong></div><div class="report-total">Desgaste<strong>${brl(p.wear)}</strong></div><div class="report-total">Custo total<strong>${brl(p.total)}</strong></div><div class="report-total">Custo/aluno<strong>${brl(p.perStudent)}</strong></div></div><div class="table-wrap"><table><thead><tr><th>Item</th><th>Protheus</th><th>Un.</th><th>Necessário</th><th>Levar</th><th>Comprar</th><th>Retorno/baixa</th><th>Preço</th><th>Total</th></tr></thead><tbody>${items.map(r=>`<tr><td><b>${esc(r.name)}</b><br><small>${esc(r.practices.join(', '))}</small></td><td>${esc(r.protheusCode||'NÃO CADASTRADO')}</td><td>${esc(r.unit)}</td><td>${r.needed}</td><td>${r.mobilize}</td><td>${r.buy}</td><td>${type==='return'?'___ devolvido / ___ danificado / ___ consumido':r.wearQty}</td><td>${r.unitPrice?brl(r.unitPrice):'Pendente'}</td><td>${brl(r.immediateCost)}</td></tr>`).join('')}</tbody></table></div><p><small>Itens sem cotação verificável não compõem o total financeiro. Validar disponibilidade, frete e especificações antes da aquisição.</small></p><p style="text-align:right;margin-top:28px"><b>Joelson M. Mendes – Esp. em Energia e IoT</b></p>`;showPage('reports')}
function safeFileName(value){return String(value||'lista-mestre').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9_-]+/g,'-').replace(/^-+|-+$/g,'').toLowerCase()}
function exportSpreadsheet(){
  if(!window.XLSX)return toast('Biblioteca de planilha não carregada. Atualize a página e tente novamente.');
  const p=state.plans.find(x=>x.id===$('#reportPlan').value);if(!p)return toast('Salve e selecione um planejamento primeiro.');
  const type=$('#reportType').value,qtyMode=$('#sheetQuantityMode')?.value||'needed',items=filteredReportItems(p,type).filter(r=>qtyMode!=='buy'||num(r.buy)>0);
  if(!items.length)return toast('Não há itens para o filtro selecionado.');
  const course=state.courses.find(c=>c.id===p.courseId)||{};
  const area='ELÉTRICA';
  const modality=String(course.modality||'FORMAÇÃO PROFISSIONAL').toUpperCase();
  const aoa=[
    [`LISTA MESTRE - ÁREA ${area}\n${modality}\nCURSO: ${p.courseName}\nCARGA HORÁRIA: ${p.hours} HORAS`,'','','','',''],
    ['','','','','',''],
    ['ITEM','CÓDIGO PRODUTO\nPROTHEUS','DESCRIÇÃO','UND.','QTD.','VALOR ESTIMADO']
  ];
  let total=0;
  items.forEach((r,i)=>{
    const qty=round(num(r[qtyMode]),2),value=round(qty*num(r.unitPrice),2);total+=value;
    aoa.push([i+1,r.protheusCode||'NÃO CADASTRADO',r.name,r.unit,qty,r.unitPrice?value:'COTAÇÃO PENDENTE']);
  });
  aoa.push(['','','','','TOTAL ESTIMADO',round(total,2)]);
  aoa.push(['','','','','','']);
  aoa.push([`Alunos: ${p.students} | Grupos: ${p.groups} | Local: ${p.location==='externo'?'Externo':'SENAI'}${p.city?' - '+p.city:''}`,'','','','','']);
  aoa.push([`Emissão: ${new Date().toLocaleDateString('pt-BR')} | Responsável: Joelson M. Mendes - Esp. em Energia e IoT`,'','','','','']);
  const ws=window.XLSX.utils.aoa_to_sheet(aoa),last=aoa.length;
  ws['!merges']=[{s:{r:0,c:0},e:{r:1,c:5}},{s:{r:last-2,c:0},e:{r:last-2,c:5}},{s:{r:last-1,c:0},e:{r:last-1,c:5}}];
  ws['!cols']=[{wch:8},{wch:22},{wch:52},{wch:10},{wch:14},{wch:20}];
  ws['!rows']=[{hpt:82},{hpt:8},{hpt:38},...items.map(()=>({hpt:24})),{hpt:28},{hpt:8},{hpt:24},{hpt:24}];
  const border={top:{style:'thin',color:{rgb:'1F2937'}},bottom:{style:'thin',color:{rgb:'1F2937'}},left:{style:'thin',color:{rgb:'1F2937'}},right:{style:'thin',color:{rgb:'1F2937'}}};
  for(let R=0;R<last;R++)for(let C=0;C<6;C++){const ref=window.XLSX.utils.encode_cell({r:R,c:C});if(!ws[ref])ws[ref]={t:'s',v:''};ws[ref].s={font:{name:'Arial',sz:10,color:{rgb:'111827'}},alignment:{vertical:'center',wrapText:true},border:R>=2&&R<=3+items.length?border:undefined};}
  ws.A1.s={fill:{fgColor:{rgb:'D9EAF7'}},font:{name:'Arial',sz:14,bold:true,color:{rgb:'0B4F8A'}},alignment:{vertical:'center',horizontal:'left',wrapText:true},border};
  for(let C=0;C<6;C++){const ref=window.XLSX.utils.encode_cell({r:2,c:C});ws[ref].s={fill:{fgColor:{rgb:'4472C4'}},font:{name:'Arial',sz:10,bold:true,color:{rgb:'FFFFFF'}},alignment:{vertical:'center',horizontal:'center',wrapText:true},border};}
  for(let R=3;R<3+items.length;R++){ws[`A${R+1}`].s.alignment.horizontal='center';ws[`B${R+1}`].s.alignment.horizontal='center';ws[`D${R+1}`].s.alignment.horizontal='center';ws[`E${R+1}`].s.alignment.horizontal='center';if(typeof ws[`F${R+1}`].v==='number')ws[`F${R+1}`].z='R$ #,##0.00';}
  const totalRow=4+items.length;ws[`E${totalRow}`].s={fill:{fgColor:{rgb:'D9EAF7'}},font:{bold:true,color:{rgb:'0B4F8A'}},alignment:{horizontal:'right',vertical:'center'},border};ws[`F${totalRow}`].s={fill:{fgColor:{rgb:'D9EAF7'}},font:{bold:true,color:{rgb:'0B4F8A'}},alignment:{horizontal:'center',vertical:'center'},border};ws[`F${totalRow}`].z='R$ #,##0.00';
  ws['!autofilter']={ref:`A3:F${3+items.length}`};
  ...
  const wb=window.XLSX.utils.book_new();window.XLSX.utils.book_append_sheet(wb,ws,'Lista Mestre');
  const file=`lista-mestre-${safeFileName(p.courseName)}-${new Date().toISOString().slice(0,10)}.xlsx`;window.XLSX.writeFile(wb,file,{cellStyles:true});toast('Planilha gerada com sucesso.');
}
...
$('#generateReportBtn').onclick=generateReport;$('#exportSpreadsheetBtn').onclick=exportSpreadsheet;$('#spreadsheetBtn').onclick=()=>{showPage('reports');setTimeout(exportSpreadsheet,80)};
